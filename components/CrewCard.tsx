...
const CrewCard: React.FC<CrewCardProps> = ({ member }) => {
  ...
  // We use inline styles or specific utility combinations based on the themeColor prop.
  ...
  return (
    <div className={`flex flex-col items-center gap-4 rounded-xl border border-white/10 bg-white/5 p-6 text-center transition-all ${theme.hoverBorder} ${theme.hoverBg}`}>
      ...
      <button className={`mt-2 flex w-full max-w-[240px] cursor-pointer items-center justify-center overflow-hidden rounded-lg ${theme.btnBg} text-sm font-bold leading-normal tracking-[0.015em] text-white transition-opacity h-10 px-4 hover:opacity-90`}>
